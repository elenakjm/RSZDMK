...
void PLS7init()
{
	...
	TCCR2A = 0x02; //tajmer 2: CTC mod
	TCCR2B = 0x03; //tajmer 2: fclk = fosc/64
	OCR2A = 249; //perioda tajmera 2: 250 Tclk (OCR2A + 1 = 250)
	TIMSK2 = 0x02; //dozvola prekida tajmera 2
	//usled dostizanja vrednosti registra OCR2A
	sei(); //I = 1 (dozvola prekida)
}

void PLS7writeDisplay(uint8_t display, uint8_t value) //setter
{
	DISP_BUFFER[display] = value;
}

uint8_t PLS7readDisplay(uint8_t display) //getter
{
